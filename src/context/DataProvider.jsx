...
const DataProvider = ({ children }) => {
  const [finalMovie, setFinalMovie] = useState([...movies]);
  const [wishlist,setWishlist] = useState([])

  const updateMoviesOnSearch = (movie) => {
    if (movie.length === 0) {
      setFinalMovie(movies);
    } else {
      const filter = movies.filter((item) =>
        item.title.toLocaleLowerCase().includes(movie.toLocaleLowerCase())
      );
      setFinalMovie([...filter]);
    }
  };

  const updateWishlist = (movieToAdd) => {
    console.log("wishlist", wishlist);    
    const finding = wishlist.find((item) => parseInt(item.id) === parseInt(movieToAdd));
    const adding = movies.find((item) => item.id === parseInt(movieToAdd));
  
    if (!finding) {
      setWishlist((prevWishlist) => [...prevWishlist, adding]);
    } else {
    }
  };
  
  const removeFromWishlist = (movieId) =>{
    const remaining = [...wishlist].filter(({id}) => id !== movieId);
    console.log('Rem',remaining)
    setWishlist([...remaining]);
  }
  
  
  const movieAddFunction= ({
    titleAdd,
    yearAdd,
    genreAdd,
    directorAdd,
  ...
  return (
    <DataContext.Provider
      value={{ movies: finalMovie, updateMoviesOnSearch: updateMoviesOnSearch,updateWishlist:updateWishlist,wishlist:wishlist,removeFromWishlist:removeFromWishlist,movieAddFunction:movieAddFunction }}
    >
      {children}
    </DataContext.Provider>
  );
